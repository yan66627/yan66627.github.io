---
permalink: /about/
title: "About Me"
author_profile: true
redirect_from:
  - /about.html
---

# Welcome!

I am **Yan Jin**, currently pursuing an **M.S. in Biostatistics** at **Washington University in St. Louis**.  
My research interests focus on:

- Aging and longevity genomics  
- Spatial transcriptomics  
- Single-cell multi-omics  
- Epigenomics & regulatory genomics  
- Statistical and computational genomics  

I am passionate about using **statistical modeling**, **probabilistic inference**, and **computational tools** to understand biological mechanisms and improve biological discoveries.

---

## 🔬 Research Interests
- Statistical Genetics & Genomics  
- Spatial transcriptomics / scRNA-seq  
- Multi-omics integration  
- EM algorithms & mixture models  
- High-dimensional data analysis  

---

## 📂 Selected Projects
### **EM Algorithm for Genotype Frequency Estimation**  
A custom R implementation of an EM algorithm estimating MAF under Hardy–Weinberg equilibrium assumptions.  
➡ See code in: `_portfolio/em_algorithm.R`

(More projects coming soon…)

---

## 📄 CV
My latest CV is available under the **CV** tab in the navigation bar.

---

## 📫 Contact
- Email: **yjin66627@gmail.com**
- GitHub: **yan66627**
- LinkedIn: *see sidebar*

Feel free to reach out for research discussions or collaboration!
